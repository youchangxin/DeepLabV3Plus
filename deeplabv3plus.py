...
def conv_layer(inputs, filters, kernel, strides, padding, activation=True, name=None):
    conv = keras.layers.Conv2D(filters=filters, kernel_size=kernel, strides=strides,
    ...
    if name is not None:
        ...
        batch_name = None
        relu_name = None
    bn = keras.layers.BatchNormalization(name=batch_name)(conv)
    if activation:
        relu = keras.layers.Activation('relu', name=relu_name)(bn)
        return relu
    else:
        return bn
...
def sep_layer(inputs, filters, strides, activation=True, name=None):
    sep_conv = keras.layers.SeparableConv2D(filters=filters, kernel_size=3,
    ...
    if name is not None:
        ...
        batch_name = None
        relu_name = None
    bn = keras.layers.BatchNormalization(name=batch_name)(sep_conv)
    if activation:
        relu = keras.layers.Activation('relu', name=relu_name)(bn)
        return relu
    else:
...
def entry_flow(inputs):
    ...
    # Instead of a max pooling layer, we do a another Third SeparableConv,
    # but with stride 2
    sep_conv3 = sep_layer(sep_conv2, 128, 2, activation=False, name='entry_sep_conv3')

    skip_conv1 = conv_layer(conv2, 128, 1, 2, 'same', activation=False, name='entry_skip_conv1')

    add1 = keras.layers.Add(name='entry_skip1')([sep_conv3, skip_conv1])
    ...
    skip_conv2 = conv_layer(add1, 256, 1, 2, 'same', activation=False, name='entry_skip_conv2')

    add2 = keras.layers.Add(name='entry_skip2')([sep_conv6, skip_conv2])
    ...
    sep_conv9 = sep_layer(sep_conv8, 728, 2, activation=False, name='entry_sep_conv9')

    skip_conv3 = conv_layer(add2, 728, 1, 2, 'same', activation=False, name='entry_skip_conv3')

    add3 = keras.layers.Add(name='entry_skip3')([sep_conv9, skip_conv3])
    # That's the entry flow finished, return the last add
    return add3, relu_add2


def middle_flow(inputs, blockname):
    # Activate the input
...
def exit_flow(inputs):
    ...
    # using Sep_conv instead of a maxpooling layer
    sep_conv3 = sep_layer(sep_conv2, 1024, 2, name='exit_sep_conv3', activation=False)
    # Skip Convolution
...
def ASPP(inputs, depthwise=False, output_stride=16):
    ...
    # The Dilated Convolutions 12x12
    if depthwise:
    ...
    # The Dilated Convolutions 18x18
    if depthwise:
...
def model(input_size=(512, 512, 3), num_classes=21, depthwise=True, output_stride=16, backbone='xception'):
    # Backbone
